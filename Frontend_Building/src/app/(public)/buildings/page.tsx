...
export default function BuildingsListPage() {
  ...
  const { buildings, loading, pagination, updateFilters, goToPage, changePageSize, isEmpty } = useBuildingPagination({
    publicMode: true,
    initialSize: 12,
  });
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-6">Danh sách văn phòng cho thuê</h1>
      <BuildingFilter onSearch={updateFilters} />
      {loading && <div className="text-center py-12">Đang tải...</div>}
      {isEmpty && <div className="text-center py-12">Không có tòa nhà phù hợp.</div>}
      {!loading && !isEmpty && (
        <>
